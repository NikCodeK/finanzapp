...
import { FixedCost, EXPENSE_CATEGORIES } from '@/lib/types';
import Input from '@/components/ui/Input';
import Select from '@/components/ui/Select';
import Button from '@/components/ui/Button';

interface FixedCostFormProps {
  onSave: (cost: Omit<FixedCost, 'id'>) => void;
  onCancel: () => void;
  initialData?: FixedCost | null;
}
...
export default function FixedCostForm({
  ...
  const [frequency, setFrequency] = useState<'monatlich' | 'vierteljaehrlich' | 'jaehrlich'>(
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Input
        label="Name"
        placeholder="z.B. Miete, Netflix, Versicherung"
        value={name}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Select
          ...
          value={frequency}
          onChange={(e) =>
            setFrequency(e.target.value as 'monatlich' | 'vierteljaehrlich' | 'jaehrlich')
          }
          options={[
